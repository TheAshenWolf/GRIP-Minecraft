    ...
    public class World : MonoBehaviour
    {
        ...
        private void BuildChunkAt(int x, int y, int z)
        {
            Vector3 chunkPosition =
                new Vector3(x * Settings.CHUNK_SIZE, y * Settings.CHUNK_SIZE, z * Settings.CHUNK_SIZE);
        ...
        {
            int i = 0;
            while (_chunksToRemove.Count > 0 && i < 1024)
            {
                string chunkName = _chunksToRemove[0];
                if (chunks.TryGetValue(chunkName, out Chunk chunk))
                {
                    Destroy(chunk.chunk);
                    chunk.Save();
        ...
        private IEnumerator SaveChangedChunks()
        {
            ...
            {
                if (chunkPair.Value.changed || Settings.SAVE_ALL_CHUNKS) chunkPair.Value.Save();
            }
        ...
        public static IEnumerator Flow(Block block, BlockType blockType, int strength, int maxSize)
        {
            ...
            if (below != null && (below.blockSetup.blockOpacity == BlockOpacity.Transparent ||
                                  below.blockSetup.blockOpacity == BlockOpacity.Liquid))
            {
        ...
        public static IEnumerator Fall(Block block, BlockType blockType)
        {
            ...
            while (isFalling)
            {
                ...
                if (previousBlock is null) thisBlock.owner.Redraw();
        ...
        {
            string worldFile = Application.persistentDataPath + "/saveData/level.dat";
            if (File.Exists(worldFile))
            {
                BinaryFormatter binaryFormatter = new BinaryFormatter();
                FileStream file = File.Open(worldFile, FileMode.Open);
                levelData = new LevelData();
                levelData = (LevelData) binaryFormatter.Deserialize(file);
                file.Close();
        ...
        public static void SaveWorldData()
        {
            ...
            if (!File.Exists(worldFile))
            {
                Directory.CreateDirectory(Path.GetDirectoryName(worldFile) ?? throw new Exception("Path is null"));
            }

            BinaryFormatter binaryFormatter = new BinaryFormatter();
            FileStream file = File.Open(worldFile, FileMode.OpenOrCreate);
            levelData = new LevelData()
            {
                amplitude = Utils.amplitude,
        ...
        {
            if (!CheckSavesDirectory()) return;
            string savesFolder = Application.persistentDataPath + "/saveData/";
        ...
        public static bool CheckWorldDataFile()
        {
            string worldFile = Application.persistentDataPath + "/saveData/level.dat";
            return File.Exists(worldFile);
        }
        
        public static bool CheckSavesDirectory()
        {
            string savesDirectory = Application.persistentDataPath + "/saveData/";
            return Directory.Exists(savesDirectory);
        }
    }
}